...
const CryptoTracker = ({ cryptoName }) => {
  ...
  // useEffect(() => {
  //   dispatch(cryptoActions.getCryptoData(cryptoName));
  // }, [dispatch, cryptoName]);
    ...
    <div className={`card ${isExpanded ? "expanded" : "collapsed"}`}>
      ...
      <div className="card-inner">
        ...
        <div className="top-data">
          <img src={cryptoName.image?.large} alt={`${name} logo`} />
          <h3 className="crypto-name">{name}</h3>
          <h4 className="crypto-price">
            {formatPrice(cryptoName.market_data?.current_price?.usd)}
            {formatPlusMinus(cryptoName.market_data?.price_change_percentage_24h)}
          </h4>
        </div>
        <ChartData isExpanded={isExpanded} cryptoName={cryptoName.id} />
      </div>
    </div>
  );
};
